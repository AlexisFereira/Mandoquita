...
export type ProductOfferProps = {
  product: Pick<
    ProductItem,
    "commerciallyAvailable" | "currency" | "price"
  >;
...
): product is ProductOfferProps["product"] & { currency: string; price: string } {
  return Boolean(
...
export function ProductOffer({ product, emphasis = "card" }: ProductOfferProps) {
  ...
  return (
    <strong className={availableClassName}>
      {product.currency.trim()} {product.price.trim()}
    </strong>
  );
}
