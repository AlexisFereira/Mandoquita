...
function PromotionalCarousel({ slides }: PromotionalCarouselProps) {
  ...
  const computedSlides = useMemo(
    () => slides.map((slide) => ({ ...slide, imageUrl: slide.imageUrl ?? "/images/banners/default-banner.svg" })),
    [slides],
  );
  ...
  function goToSlide(nextIndex: number) {
    ...
    setActiveIndex(((nextIndex % computedSlides.length) + computedSlides.length) % computedSlides.length);
  }
  ...
  return (
    <section
      ...
      onBlurCapture={(event) => {
        if (!event.currentTarget.contains(event.relatedTarget as Node | null)) setIsFocusedWithin(false);
      }}
    >
      <div
        className="flex"
          ...
          <article key={slide.title} className="relative" style={{ width: `${100 / computedSlides.length}%`, flex: `0 0 ${100 / computedSlides.length}%` }}>
            <div className="relative h-[clamp(220px,44vw,520px)] w-full overflow-hidden">
              ...
              <img
                ...
                onLoad={() => setLoadedSlides((previous) => ({ ...previous, [index]: true }))}
...
function GalleryCarousel({
  ...
  const [uncontrolledId, setUncontrolledId] = useState(defaultActiveItemId ?? items[0]?.id);
  ...
  const activeIndex = Math.max(0, items.findIndex((item) => item.id === requestedId));
  const activeItem = items[activeIndex];

  useEffect(() => {
    if (!isControlled && items.length > 0 && !items.some((item) => item.id === uncontrolledId)) {
  ...
  return (
    <section aria-label={ariaLabel} className="w-full space-y-4">
      <div className="relative flex aspect-square items-center justify-center overflow-hidden rounded-lg border border-[rgb(var(--border)/1)] bg-[rgb(var(--surface-muted)/1)]">
        {!loadedIds[activeItem.id] && !failed ? <span aria-hidden="true" className="absolute inset-0 animate-pulse bg-[rgb(var(--muted)/0.12)] motion-reduce:animate-none" /> : null}
        {failed ? (
          <p className="p-6 text-center text-[rgb(var(--muted)/1)]">{failedMediaMessage}</p>
        ) : (
          <img
            key={activeItem.id}
            src={activeItem.src}
            ...
            onLoad={() => setLoadedIds((previous) => ({ ...previous, [activeItem.id]: true }))}
            onError={() => setFailedIds((previous) => ({ ...previous, [activeItem.id]: true }))}
          />
        )}
      </div>
      ...
      {items.length > 1 ? (
        <>
          <p className="text-sm text-[rgb(var(--muted)/1)]" aria-live="polite" aria-atomic="true">Imagen {activeIndex + 1} de {items.length}.</p>
          <div role="group" className="flex flex-wrap gap-2" aria-label="Seleccionar imagen">
            {items.map((item, index) => {
              ...
              return (
                <button
                  ...
                  aria-label={item.controlLabel ?? `Mostrar imagen ${index + 1} de ${items.length}`}
                  ...
                  style={{ borderColor: selected ? "rgb(var(--primary) / 1)" : "rgb(var(--border) / 1)", borderWidth: selected ? "3px" : "1px" }}
                >
                  {showThumbnails ? <img src={item.thumbnailSrc ?? item.src} alt="" aria-hidden="true" width="56" height="56" className="h-12 w-12 object-contain" /> : <span className="px-2">{index + 1}</span>}
                  {selected ? <span className="sr-only">Imagen actual</span> : null}
                </button>
              );
            })}
          </div>
...
export function Carousel(props: CarouselProps) {
  return props.mode === "gallery" ? <GalleryCarousel {...props} /> : <PromotionalCarousel {...props} />;
}
