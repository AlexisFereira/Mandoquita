...
export function createProductStructuredData(
  item: ProductDetailResponse["item"],
) {
...
export default function ProductDetailPage({
  ...
  const initialImageId =
    item.images.find((image) => image.isPrimary)?.id ?? item.images[0]?.id;
  ...
  const handleVariantResolved = useCallback(
    (variant: PublicProductVariantItem) => {
      if (
        variant.imageId &&
        item.images.some((image) => image.id === variant.imageId)
      ) {
        ...
        <Container size="wide" padding="lg">
          ...
          <article className="mb-12 grid gap-10 lg:grid-cols-[minmax(0,1.1fr)_minmax(320px,0.9fr)] lg:gap-12">
            ...
            <div className="flex self-start flex-col gap-5">
              ...
              <h1 className="ds-heading ds-heading-lg">{item.name}</h1>
          ...
          {item.description ? (
            ...
            >
              <h2
                ...
                className="ds-heading ds-heading-md mb-4"
              >
                Descripción
              </h2>
          ...
          {hasMerchandisingMetadata ? (
            <section
              aria-labelledby="product-metadata-heading"
              className="mb-10 max-w-3xl"
            >
              <h2
                id="product-metadata-heading"
                className="ds-heading ds-heading-md mb-4"
              >
              ...
              {item.brand || item.collection || genderLabel ? (
                <dl className="m-0 grid gap-2 text-sm">
                  ...
                  {item.collection ? (
                    <div className="flex flex-wrap gap-2">
                      <dt className="font-semibold">Colección:</dt>
                      <dd className="m-0">{item.collection}</dd>
                    </div>
                  ) : null}
                  {genderLabel ? (
                    <div className="flex flex-wrap gap-2">
                      <dt className="font-semibold">Aplicabilidad:</dt>
                      <dd className="m-0">{genderLabel}</dd>
                    </div>
                  ) : null}
                </dl>
              ) : null}
