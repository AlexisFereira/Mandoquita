...
import { restoreProductType } from "../../../../../src/server/modules/productTypes";
import {
...
export async function handleAdminProductTypeRestore(
  req: NextApiRequest,
  res: NextApiResponse,
) {
  ...
  try {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      const item = await restoreProductType(txRepo, name, req.body);
      await tx.productAdminAuditEvent.create({
        data: {
          requestId: productAdminRequestId(req),
          event: "PRODUCT_TYPE_RESTORE",
          outcome: "SUCCESS",
...
export default handleAdminProductTypeRestore;
