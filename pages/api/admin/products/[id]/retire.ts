...
import {
  parseProductId, ProductNotFoundError, ProductUpdateConflictError, retireProduct,
...
export async function handleProductRetirement(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    const item = await prisma.$transaction(async (tx) => {
      const result = await retireProduct(tx, productId, req.body, authorized.account.id);
      await tx.productAdminAuditEvent.create({ data: {
        requestId: productAdminRequestId(req), event: "PRODUCT_RETIRE", outcome: "SUCCESS",
    ...
    if (error instanceof ProductUpdateConflictError ||
      ...
      return res.status(409).json({ error: "Product retirement conflicts with current state" });
    }
    return respondProductAdminError(res, error);
  }
}

export default handleProductRetirement;
