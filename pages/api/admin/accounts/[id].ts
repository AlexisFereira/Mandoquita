...
export async function handleAdminAccount(req: NextApiRequest, res: NextApiResponse) {
  ...
  try {
    ...
    if (!targetId) return res.status(400).json({ error: "Invalid account ID" });
    if (targetId === authorized.account.id) return res.status(409).json({ error: "The protected Superadministrator cannot be changed here" });
    ...
    if (input.action === "deactivate" && !target.enabled) return res.status(409).json({ error: "Administrator is already inactive" });
    if (input.action === "reactivate" && target.enabled) return res.status(409).json({ error: "Administrator is already active" });
    if (input.action === "reset" && !target.enabled) return res.status(409).json({ error: "Inactive Administrator must be reactivated" });
