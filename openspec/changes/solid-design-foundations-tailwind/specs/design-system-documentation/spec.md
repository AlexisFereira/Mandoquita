...
The system SHALL create a markdown file at `src/design-system/DESIGN_TOKENS.md` that documents all available design tokens with their values, intended use, and examples. The guide SHALL include sections for colors (with hex values and light/dark variants), spacing scale, typography, shadows, and border radius.
...
- **AND** light and dark variants are shown side-by-side
...
### Requirement: Dark mode usage is documented

The system SHALL document how to ensure components work correctly in both light and dark modes. Documentation SHALL include guidelines for using design tokens, testing both themes, and handling theme-specific logic.

#### Scenario: Developer learns dark mode support

- **WHEN** developer reads integration guide
- **THEN** section explains that CSS variables automatically handle theme switching
- **AND** developers are reminded to use semantic color tokens, not hardcoded hex
- **AND** testing recommendations are provided (check both light and dark modes)
